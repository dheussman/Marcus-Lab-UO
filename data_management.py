...
class MidpointNormalize(cols.Normalize):
	"""
	...
	def __init__(self, vmin=None, vmax=None, midpoint=None, clip=False):
		...
		cols.Normalize.__init__(self, vmin, vmax, clip)
...
class Data_2D(Data):
    ...
    def __init__(self, rp_dat, nrp_dat, axes = (npa(range(0, 30, 1))*2.66e-15, npa(range(0, 30, 1))*2.66e-15), tag = None, bad = False):
        ...
        self.rp_dat -= self.rp_dat[29,29]
        self.nrp_dat -= self.nrp_dat[29,29]
...
#f        self.ft_axes = [rp21, nrp43]     ###want to flip both 21 axes or just the difference?
        self.ft_axes_nrp = [ax1, ax2]
        self.ft_axes_rp = [-ax1, ax2]
        self.normalize()
    ...
    def simple_plot(self, filename = None, title = None):
        ...
        plt.show()
    def compare_plot_with(self, other, style = "interferogram", filename = None):
        '''create a side by side plot of two data objects'''
    ...
    def truncate_colormap(self, cmap, minval=0.0, maxval=1.0, n=100):
        ...
        return new_cmap
    def plot_with_tc(self, mc = 17532, title = None, filename = None):
...
#            return new_cmap
        ...
        colormap = self.truncate_colormap(cmap2, -2, 2) 
        #DEBUG
...
#        x, y = self.ft_axes
        ...
        #Interfereograms
        rp_int = fig1.add_subplot(331)
        nrp_int = fig1.add_subplot(332)
        
        #Transforms
        rp_ftr = fig1.add_subplot(334)
        rp_fti = fig1.add_subplot(337)
        
        nrp_ftr = fig1.add_subplot(335)
        nrp_fti = fig1.add_subplot(338)
        
        tc_r = fig1.add_subplot(336)
        tc_i = fig1.add_subplot(339)
        
        # labels
        #rp_int.set_xlabel("RP Interferogram")
        #nrp_int.set_ylabel("NRP Interferogram")
        #rp_ftr.set_xlabel("Fourier Transform (Real)")
        rp_ftr.set_ylabel("FT - Real")
        #nrp_ftr.set_xlabel("Fourier Transform (Real)")
        rp_fti.set_ylabel("FT - Imag")
        rp_fti.set_xlabel("Rephasing")
        nrp_fti.set_xlabel("Nonrephasing")
        tc_i.set_xlabel("Total Correlation")
        # Contour Plots
        elev_min=-1000
        elev_max=3000
        mid_val=0
        
        rp_min = np.amin(np.imag(self.rp_dat))
        rp_max = np.amax(np.imag(self.rp_dat))
        print(rp_min, rp_max)
        
        nrp_min = np.amin(np.imag(self.nrp_dat))
        nrp_max = np.amax(np.imag(self.nrp_dat))

        cb = rp_int.matshow(np.imag(self.rp_dat), cmap = "PuOr", clim=(rp_min, rp_max), norm=MidpointNormalize(midpoint=mid_val,vmin=elev_min, vmax=elev_max))
        plt.colorbar(cb, ax = rp_int)
        #rp_int.title.set_text('Rephasing')
        rp_int.set_anchor('W')
        
        cb = nrp_int.matshow(np.imag(self.nrp_dat), cmap = "PuOr", clim=(nrp_min, nrp_max), norm=MidpointNormalize(midpoint=mid_val,vmin=elev_min, vmax=elev_max))
        plt.colorbar(cb, ax = nrp_int)
        nrp_int.yaxis.set_visible(False)
        nrp_int.set_anchor('W')

        #rp_ftr.gca().set_aspect('equal', adjustable='box')
        rp_ftr.contourf(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, (np.real(self.ft_rp)), levels = levels1, cmap = cmap2, norm=MidpointNormalize(midpoint=0,vmin=1.1*np.amin(np.real(self.ft_rp)), vmax=1.1*np.amax(np.real(self.ft_rp))))
        ...
        clrb = plt.contourf(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, np.real(self.ft_rp),cmap='rainbow')
        plt.colorbar(clrb, ax = rp_ftr)
        CS = rp_ftr.contour(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, (np.real(self.ft_rp)), colors='black',levels=levelscon, linewidths=1.5, alpha=1)
        for line in CS.collections:
            if line.get_linestyle() != [(None, None)]: 
                line.set_color('white')
                line.set_linewidth(1.5)
                line.set_dashes('solid')
        rp_ftr.xaxis.set_visible(False)
        

        rp_fti.contourf(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, (np.imag(self.ft_rp)), levels = levels1, cmap = cmap2, norm=MidpointNormalize(midpoint=0,vmin=1.1*np.amin(np.imag(self.ft_rp)), vmax=1.1*np.amax(np.imag(self.ft_rp))))
#        rp_fti.contour(self.ft_axes[0], self.ft_axes[1], (np.imag(self.ft_rp)), n_cont)
        ...
        clrb = plt.contourf(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, np.imag(self.ft_rp),cmap='rainbow')
        plt.colorbar(clrb, ax = rp_fti)
        #plt.colorbar(qcs)        
        CS = rp_fti.contour(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, (np.imag(self.ft_rp)), colors='black',levels=levelscon, linewidths=1.5, alpha=1)
        for line in CS.collections:
            if line.get_linestyle() != [(None, None)]: 
                line.set_color('white')
                line.set_linewidth(1.5)
        ...
        nrp_ftr.contourf(self.ft_axes_nrp[0]+mc, self.ft_axes_nrp[1]+mc, (np.real(self.ft_nrp)), levels = levels1, cmap = cmap2, norm=MidpointNormalize(midpoint=0,vmin=1.1*np.amin(np.real(self.ft_nrp)), vmax=1.1*np.amax(np.real(self.ft_nrp))))
#        nrp_ftr.contour(self.ft_axes[0], self.ft_axes[1], np.flipud((np.real(self.ft_nrp))), n_cont)
        ...
        clrb = plt.contourf(self.ft_axes_nrp[0]+mc,self.ft_axes_nrp[1]+mc,np.real(self.ft_nrp),cmap='rainbow')
        plt.colorbar(clrb, ax = nrp_ftr)
        CS = nrp_ftr.contour(self.ft_axes_nrp[0]+mc, self.ft_axes_nrp[1]+mc, (np.real(self.ft_nrp)), colors='black',levels=levelscon, linewidths=1.5, alpha=1)
        for line in CS.collections:
            if line.get_linestyle() != [(None, None)]: 
                line.set_color('white')
                line.set_linewidth(1.5)
                line.set_dashes('solid')
        nrp_ftr.xaxis.set_visible(False)
        nrp_ftr.yaxis.set_visible(False)

        nrp_fti.contourf(self.ft_axes_nrp[0]+mc, self.ft_axes_nrp[1]+mc, (np.imag(self.ft_nrp)), levels = levels1, cmap = cmap2, norm=MidpointNormalize(midpoint=0,vmin=1.1*np.amin(np.imag(self.ft_nrp)), vmax=1.1*np.amax(np.imag(self.ft_nrp))))
#        nrp_fti.contour(self.ft_axes[0], self.ft_axes[1], np.flipud(np.imag(self.ft_nrp)), n_cont)
        ...
        #plt.colorbar(qcs)
        clrb = plt.contourf(self.ft_axes_nrp[0]+mc,self.ft_axes_nrp[1]+mc,np.imag(self.ft_nrp),cmap='rainbow')
        plt.colorbar(clrb, ax = nrp_fti)
        CS = nrp_fti.contour(self.ft_axes_nrp[0]+mc, self.ft_axes_nrp[1]+mc, (np.imag(self.ft_nrp)), colors='black',levels=levelscon, linewidths=1.5, alpha=1)
        for line in CS.collections:
            if line.get_linestyle() != [(None, None)]: 
                line.set_color('white')
                line.set_linewidth(1.5)
                line.set_dashes('solid')
        nrp_fti.contourf(self.ft_axes_nrp[0]+mc, self.ft_axes_nrp[1]+mc, (np.imag(self.ft_nrp)), levels = levels1, cmap = cmap2, norm=MidpointNormalize(midpoint=0,vmin=1.1*np.amin(np.imag(self.ft_nrp)), vmax=1.1*np.amax(np.imag(self.ft_nrp))))
        nrp_fti.yaxis.set_visible(False)
        
        ########################     TOTAL CORRELATION       #####################
        
        tc_r.contourf(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, (np.real(self.ft_rp) + np.fliplr(np.real(self.ft_nrp)))/2, levels = levels1, cmap = cmap2, norm=MidpointNormalize(midpoint=0,vmin=1.1*np.amin(np.real(self.ft_rp)), vmax=1.1*np.amax(np.real(self.ft_rp))))
        tc_r.plot(tc_r.get_xlim(), tc_r.get_ylim(), ls="--", c=".2")
        tc_r.plot((tc_r.get_xlim()[0], tc_r.get_ylim()[-1]), (tc_r.get_xlim()[-1], tc_r.get_ylim()[0]), ls="--", c=".2")
        tc_r.set_xlim(xlims)
        tc_r.set_ylim(ylims)
        clrb = plt.contourf(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, (np.real(self.ft_rp) + np.fliplr(np.real(self.ft_nrp)))/2,cmap='rainbow')
        plt.colorbar(clrb, ax = tc_r)
        CS = tc_r.contour(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, (np.real(self.ft_rp) + np.fliplr(np.real(self.ft_nrp)))/2, colors='black',levels=levelscon, linewidths=1.5, alpha=1)
        for line in CS.collections:
            if line.get_linestyle() != [(None, None)]: 
                line.set_color('white')
                line.set_linewidth(1.5)
                line.set_dashes('solid')
        tc_r.xaxis.set_visible(False)
        tc_r.yaxis.set_visible(False)
        
        tc_i.contourf(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, (np.imag(self.ft_rp) + np.fliplr(np.imag(self.ft_nrp)))/2, levels = levels1, cmap = cmap2, norm=MidpointNormalize(midpoint=0,vmin=1.1*np.amin(np.imag(self.ft_rp)), vmax=1.1*np.amax(np.imag(self.ft_rp))))
#        tc_i.contour(self.ft_axes[0], self.ft_axes[1], (np.imag(self.ft_rp)), n_cont)
        tc_i.plot(tc_i.get_xlim(), tc_i.get_ylim(), ls="--", c=".2")
        tc_i.plot((tc_i.get_xlim()[0], tc_i.get_ylim()[-1]), (tc_i.get_xlim()[-1], tc_i.get_ylim()[0]), ls="--", c=".2")
        tc_i.set_xlim(xlims)
        tc_i.set_ylim(ylims)
        clrb = plt.contourf(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, (np.imag(self.ft_rp) + np.fliplr(np.imag(self.ft_nrp)))/2, cmap='rainbow')
        plt.colorbar(clrb, ax = tc_i)
        #plt.colorbar(qcs)        
        CS = tc_i.contour(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, (np.imag(self.ft_rp) + np.fliplr(np.imag(self.ft_nrp)))/2, colors='black',levels=levelscon, linewidths=1.5, alpha=1)
        for line in CS.collections:
            if line.get_linestyle() != [(None, None)]: 
                line.set_color('white')
                line.set_linewidth(1.5)
                line.set_dashes('solid')
        tc_i.yaxis.set_visible(False)
        
        
        #fig1.subplots_adjust(right=0.81)
        #cbar_ax = fig1.add_axes([0.85, 0.15, 0.05, 0.7])
        #cbar = cbar_ax.matshow(npa([-1*np.arange(-1,1,.01) for i in range(2)]).T, cmap = cmap2, clim=(-1, 1), norm=MidpointNormalize(midpoint=mid_val,vmin=1, vmax=-1), extent=[1,0,-8,10])
        #cbar_ax.set_ylabel("Signal Amplitude (arb.)")
        
        #redo to fix weirdness...
        tc_i.contourf(self.ft_axes_rp[0]+mc, self.ft_axes_rp[1]+mc, (np.imag(self.ft_rp) + np.fliplr(np.imag(self.ft_nrp)))/2, levels = levels1, cmap = cmap2, norm=MidpointNormalize(midpoint=0,vmin=1.1*np.amin(np.imag(self.ft_rp)), vmax=1.1*np.amax(np.imag(self.ft_rp))))
        ...
        plt.show()
    def super_plot(self, mc = 17532, title = None, filename = None):
        '''Plots both interferograms and FTs'''
        # let's begin by defining our contours
#        def truncate_colormap(cmap, minval=0.0, maxval=1.0, n=100):
#            new_cmap = cols.LinearSegmentedColormap.from_list(
...
#        x, y = self.ft_axes
        ...
        rp_int.matshow(np.imag(self.rp_dat), cmap = "PuOr", clim=(rp_min, rp_max), norm=MidpointNormalize(midpoint=mid_val,vmin=elev_min, vmax=elev_max))
        nrp_int.matshow(np.imag(self.nrp_dat), cmap = "PuOr", clim=(nrp_min, nrp_max), norm=MidpointNormalize(midpoint=mid_val,vmin=elev_min, vmax=elev_max))
...
def import_2D_data(directory, suffix, monochrom1, monochrom2 = None, time_step = 2.66e-15, make_plots = False, bad1 = False):
    ...
    rp_dat = (rp_phase * rp).T
    rp_dat *= (1/rp_dat[0,0])
    
    nrp_dat = (nrp_phase * nrp).T
    nrp_dat *= (1/nrp_dat[0,0])
    
    
    #debugging
    data = Data_2D(rp_dat, nrp_dat, bad = bad1)
